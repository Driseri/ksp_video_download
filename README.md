Этот инструмент предназначен только для загрузки видео из открытых/свободно лицензированных источников; использование для обхода платного доступа запрещено.
...
**Особенность:** Приложение включает специализированный функционал для загрузки видео с платформы **Платформа K** с поддержкой JSON-конфигураций и настраиваемых referrer-заголовков.

## 🎬 Как скачать видео с Платформа K (для пользователей с доступом)

Если у вас есть доступ к видео на Платформа K и вы хотите его скачать:

1.  **Перейдите на страницу с нужным видео** в вашем браузере.
2.  **Начните просмотр видео**.
3.  **Кликните правой кнопкой мыши по видео** и выберите "Сохранить системный журнал" (или аналогичную опцию, которая сохраняет информацию о видеопотоке).
4.  **Откройте вкладку "Платформа K"** в приложении KSP Video Downloader.
5.  **Перетащите сохраненный файл** (обычно это JSON-файл) в область Drag & Drop или выберите его через проводник.
6.  Если видео доступно для скачивания, **появится зеленое уведомление**.
7.  **Выберите желаемые настройки скачивания** (качество, формат, кодек).
8.  **Нажмите кнопку "Загрузить"**.
9.  **Дождитесь завершения загрузки файла**.
...
![Интерфейс загрузки Платформа K](images/k_down.png)

*Специализированный интерфейс для загрузки видео с Платформа K, включающий поддержку JSON-конфигураций и настройку referrer-заголовков для обхода ограничений доступа.*
...
### 🎬 Специальная поддержка Платформа K
- **Прямая загрузка** с платформы Платформа K
...
### Загрузка с Платформа K
1. Перейдите на вкладку "Платформа K"
...
*[Здесь должен быть скриншот интерфейса Платформа K]*

### Пример JSON для Платформа K
```json
{
  "referrer": "https://Платформа K.io/embed/123456789",
  "video_data": {
...
|-------------|---------------------|----------------|
| **Платформа K поддержка** | ✅ Встроенная | ❌ Отсутствует |
